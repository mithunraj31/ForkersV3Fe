    ...
    <el-row>
      <el-col class="tool-bar" :span="24">
        <el-button @click="dialogFormVisible = true" type="primary"
          >New chart</el-button
        >
        <el-button @click="isEditable = !isEditable" type="warning"
          >edit mode: {{ isEditable ? "On" : "Off" }}</el-button
        >
      </el-col>
    </el-row>
    <el-row :gutter="5">
      <el-col class="col-layout">
        <el-card class="box-card" shadow="never">
          <h3>
            オペレータ毎の総アラーム数、総稼働時間、運転スコア（リスト表示）​
          </h3>
        ...
        >
          <el-card class="box-card" :shadow="isEditable ? 'hover' : 'never'">
            <el-button
              ...
              class="delete-chart"
              type="danger"
              @click="onDeleteChartClicked(chart.id)"
              >X</el-button
            >
            <component
              :is="chart.chartKey"
    ...
    <el-dialog title="New Chart" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        ...
        <el-form-item label="size" :label-width="formLabelWidth">
          <el-select
            v-model="form.size"
            placeholder="Please select a chart size"
          >
  ...
  data() {
    return {
      ...
      chartTemplates: [
        ...
        {
          name:
            "期間毎の車両グループ毎のアラーム数と全車両グループの総稼働時間​",
